...
async def create_course(
    ...
    if current_user.role == 'teacher':
        if not school_id:
            return error_response(message="教师账号未关联机构", code=400)
        # 强制使用教师的机构ID
        course_data.school_id = school_id
    elif current_user.role == 'teacher':
        # 教师只能创建自己机构的课程
        if course_data.school_id != school_id:
            return error_response(message="无权创建其他机构的课程", code=403)
...
async def get_course(
    ...
    # 权限检查
    if current_user.role == 'teacher' and current_user.school_id != course.school_id:
...
async def enroll_course(
    ...
    elif current_user.role not in ['platform_admin', 'teacher', 'student']:
...
async def create_device_authorization(
    ...
    """为课程授权设备组（教师操作）"""
    # 权限检查：只有教师可以授权
    if current_user.role != 'teacher' and current_user.role != 'platform_admin':
        return error_response(message="只有教师才能授权设备", code=403)
...
async def update_device_authorization(
    ...
    if current_user.role not in ['platform_admin', 'teacher']:
        return error_response(message="只有教师才能修改授权", code=403)
...
async def revoke_device_authorization(
    ...
    # 权限检查
    if current_user.role not in ['platform_admin', 'teacher']:
        return error_response(message="只有教师才能撤销授权", code=403)
