...
from app.schemas.user_management import (
    TeacherCreate, StudentCreate, AssignRoleRequest,
...
def check_teacher_or_teacher(current_user: User = Depends(get_current_user)):
    """检查当前用户是否为教师或教师"""
    if current_user.role not in ['platform_admin', 'teacher', 'teacher']:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="只有教师或教师才能执行此操作"
        )
    return current_user

def check_teacher(current_user: User = Depends(get_current_user)):
    """检查当前用户是否为教师"""
    if current_user.role not in ['platform_admin', 'teacher']:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="只有教师才能执行此操作"
        )
...
async def list_users(
    ...
    school_id: Optional[int] = Query(None, description="机构ID筛选"),
    course_id: Optional[int] = Query(None, description="课程ID筛选（学生）"),
    keyword: Optional[str] = Query(None, description="搜索关键词"),
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    db: Session = Depends(get_db),
    current_user: User = Depends(check_teacher_or_teacher)
):
    """
    获取用户列表（支持多条件筛选）
    用于分组管理等场景
...
async def create_teacher(
    ...
    # 获取机构ID
    if current_user.role == 'platform_admin':
        # 平台管理员需要指定机构（这里暂不支持，让其通过创建教师的方式）
        return error_response(
            message="平台管理员请先创建教师，再由教师创建教师",
            code=400
        )
    ...
    if not school:
        return error_response(message="机构不存在", code=404)
    ...
    if existing_number:
        return error_response(message="工号在该机构已存在", code=400)
    
    # 检查是否超过最大教师数
    teacher_count = db.query(func.count(User.id)).filter(
        User.school_id == school_id,
        User.role.in_(['teacher', 'teacher']),
...
async def create_student(
    ...
    if existing_number:
        return error_response(message="学号在该机构已存在", code=400)
...
async def assign_role(
    ...
    if user.role != 'individual' or user.school_id is not None:
        return error_response(
            message="只能将独立用户转换为教师、学生或教师",
            code=400
        )
    
    # 获取机构ID（如果是设置教师，使用请求中的school_id；否则使用当前用户的school_id）
    if assign_request.new_role == 'teacher':
        # 平台管理员可以设置任意机构的管理员
        if current_user.role != 'platform_admin':
            return error_response(
                message="只有平台管理员才能设置教师",
                code=403
            )
    ...
    if assign_request.new_role == 'teacher':
    ...
    elif assign_request.new_role == 'teacher':
        # 检查工号是否已存在
        existing_number = db.query(User).filter(
            User.school_id == school_id,
            User.teacher_number == assign_request.teacher_number,
        ...
        user.school_name = school.school_name
        user.teacher_number = assign_request.teacher_number
        user.subject = assign_request.subject  # 教师也可以有学科
...
async def batch_import_teachers(
    ...
    # 权限检查
    if current_user.role not in ['platform_admin', 'teacher']:
        return error_response(message="只有教师才能批量导入教师", code=403)
...
async def batch_import_students(
    ...
    # 权限检查
    if current_user.role not in ['platform_admin', 'teacher']:
        return error_response(message="只有教师才能批量导入学生", code=403)
