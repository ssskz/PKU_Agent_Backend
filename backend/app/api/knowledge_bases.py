...
def is_admin_user(user: User) -> bool:
    """检查是否是管理员"""
    return user.role in ['platform_admin', 'teacher']
...
def get_kb_school_id(kb: KnowledgeBase, db: Session) -> Optional[int]:
    ...
    if kb.scope_type == 'school':
        ...
        # 个人知识库：通过scope_id（用户ID）获取用户的机构ID
...
async def create_knowledge_base(
    ...
    if kb_data.scope_type == 'system':
        ...
        if current_user.role not in ['platform_admin', 'teacher']:
            return error_response(message="只有平台管理员或教师可以创建机构级知识库", code=403)
        ...
        if kb_data.scope_id:
            ...
            if current_user.role == 'teacher' and course.school_id != current_user.school_id:
    ...
    try:
        ...
        if current_user.school_id:
            ...
            try:
                ...
                logger.error(f"[可用知识库] 查询机构级知识库失败: {e}")
